...
const ICAREnv = () => {
  ...
  const texture = useTexture('/Spidy%20images/SPIDERMAN7.png');
  ...
  useFrame((state) => {
    ...
    if (cardMeshRef.current) {
      // Animation Profile [Gliding Tilt]
      cardMeshRef.current.rotation.z = Math.sin(time * 1.8) * 0.08;
    }
  ...
  return (
    <group ref={groupRef} scale={[0.001, 0.001, 0.001]}>
      ...
      <Billboard follow={true}>
        <mesh 
          ref={cardMeshRef}
          onClick={(e) => { e.stopPropagation(); setActiveProject('icar'); }}
        ...
        <group ref={labelRef} position={[0, 4.2, 0.3]}>
          <Text anchorX="center" anchorY="middle" color={hovered ? "#ff0055" : "#00f0ff"} fontSize={0.4} letterSpacing={0.08}>
            {"[ CLICK TO BIND ICAR-NRCY RESEARCH FIELD METRICS ]"}
          </Text>
        </group>
      </Billboard>
    </group>
