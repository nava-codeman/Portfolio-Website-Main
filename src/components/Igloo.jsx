...
const Igloo = () => {
  ...
  const texture = useTexture('/Spidy%20images/SPIDERMAN2.png');
  ...
  useFrame((state) => {
    ...
    if (cardMeshRef.current) {
      // Animation Profile [Breathing Scale]
      const s = 1 + Math.sin(time * 2.0) * 0.03;
  ...
  return (
    <group ref={groupRef} scale={[0.001, 0.001, 0.001]}>
      ...
      <Billboard follow={true}>
        <mesh 
          ref={cardMeshRef}
          onClick={(e) => { e.stopPropagation(); setActiveProject('overview'); }}
        ...
        <group ref={labelRef} position={[0, 4.2, 0.3]}>
          <Text anchorX="center" anchorY="middle" color={hovered ? "#ff0055" : "#00f0ff"} fontSize={0.4} letterSpacing={0.08}>
            {"[ CLICK TO INITIALIZE CONTEXT ENGAGEMENT MODULE ]"}
          </Text>
        </group>
      </Billboard>
    </group>
