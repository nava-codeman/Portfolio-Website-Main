...
const EducationEnv = () => {
  ...
  // Load relative path correctly using standard spaces string mapping
  const texture = useTexture('/Spidy%20images/SPIDERMAN3.png');
  ...
  useFrame((state) => {
    ...
    if (cardMeshRef.current) {
      // Animation Profile [Continuous Smooth Rotate]
      cardMeshRef.current.rotation.y = time * 0.4;
    }

    if (ringRef.current) {
      ringRef.current.rotation.z = time * 0.6;
      ringRef.current.material.opacity = 0.3 + Math.sin(time * 3.5) * 0.15;
    }
    if (labelRef.current) {
  ...
  return (
    <group ref={groupRef} scale={[0.001, 0.001, 0.001]}>
      {/* TARGET LAYER: Pulsing Base Glow Circle */}
      ...
      {/* RENDER GRID: Large Camera-Facing Display Card */}
      <Billboard follow={true}>
        <mesh 
          ref={cardMeshRef}
          onClick={(e) => { e.stopPropagation(); setActiveProject('adbu'); }}
        ...
        {/* UX COMPONENT: Large Action Prompt Text Overlay */}
        <group ref={labelRef} position={[0, 4.2, 0.3]}>
          <Text anchorX="center" anchorY="middle" color={hovered ? "#ff0055" : "#00f0ff"} fontSize={0.4} letterSpacing={0.08}>
            {"[ CLICK TO ACCESS ACADEMIC & INSTITUTIONAL HISTORY ]"}
          </Text>
        </group>
      </Billboard>
    </group>
