...
const FitHubEnv = () => {
  ...
  const texture = useTexture('/Spidy%20images/SPIDERMAN5.png');
  ...
  useFrame((state) => {
    ...
    if (cardMeshRef.current) {
      // Animation Profile [Figure-8 Orbit]
      cardMeshRef.current.position.x = Math.sin(time * 2) * 0.3;
      cardMeshRef.current.position.y = Math.sin(time) * 0.2;
    }
  ...
  return (
    <group ref={groupRef} scale={[0.001, 0.001, 0.001]}>
      ...
      <Billboard follow={true}>
        <mesh 
          ref={cardMeshRef}
          onClick={(e) => { e.stopPropagation(); setActiveProject('fithub'); }}
        ...
        <group ref={labelRef} position={[0, 4.2, 0.3]}>
          <Text anchorX="center" anchorY="middle" color={hovered ? "#ff0055" : "#00f0ff"} fontSize={0.4} letterSpacing={0.08}>
            {"[ CLICK TO RUN FITHUB WORKOUT & ECOSYSTEM FRAMEWORK ]"}
          </Text>
        </group>
      </Billboard>
    </group>
