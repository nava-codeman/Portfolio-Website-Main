...
const FundSureEnv = () => {
  ...
  const texture = useTexture('/Spidy%20images/SPIDERMAN6.png');
  ...
  useFrame((state) => {
    ...
    if (cardMeshRef.current) {
      // Animation Profile [Wave Glitch Distortion]
      cardMeshRef.current.material.opacity = 0.88 + Math.sin(time * 12) * 0.07;
    }
  ...
  return (
    <group ref={groupRef} scale={[0.001, 0.001, 0.001]}>
      ...
      <Billboard follow={true}>
        <mesh 
          ref={cardMeshRef}
          onClick={(e) => { e.stopPropagation(); setActiveProject('fundsure'); }}
        ...
        <group ref={labelRef} position={[0, 4.2, 0.3]}>
          <Text anchorX="center" anchorY="middle" color={hovered ? "#ff0055" : "#00f0ff"} fontSize={0.4} letterSpacing={0.08}>
            {"[ CLICK TO MOUNT FUNDSURE LIVE ALLOCATION ROUTER ]"}
          </Text>
        </group>
      </Billboard>
    </group>
