...
const LeadershipEnv = () => {
  ...
  const texture = useTexture('/Spidy%20images/SPIDERMAN11.png');
  ...
  useFrame((state) => {
    ...
    if (cardMeshRef.current) {
      // Animation Profile [Vertical Bounce]
      cardMeshRef.current.position.y = Math.abs(Math.sin(time * 3)) * 0.8;
    }
  ...
  return (
    <group ref={groupRef} scale={[0.001, 0.001, 0.001]}>
      ...
      <Billboard follow={true}>
        <mesh 
          ref={cardMeshRef}
          onClick={(e) => { e.stopPropagation(); setActiveProject('leadership'); }}
        ...
        <group ref={labelRef} position={[0, 4.2, 0.3]}>
          <Text anchorX="center" anchorY="middle" color={hovered ? "#ff0055" : "#00f0ff"} fontSize={0.4} letterSpacing={0.08}>
            {"[ CLICK TO EXTRACT EXECUTIVE SPORTS COMMITTEE CHRONICLES ]"}
          </Text>
        </group>
      </Billboard>
    </group>
