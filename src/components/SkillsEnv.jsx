...
const SkillsEnv = () => {
  ...
  const texture = useTexture('/Spidy%20images/SPIDERMAN10.png');
  ...
  useFrame((state) => {
    ...
    if (cardMeshRef.current) {
      // Animation Profile [Wobble Orbit]
      cardMeshRef.current.position.z = Math.sin(time * 2) * 0.5;
    }
  ...
  return (
    <group ref={groupRef} scale={[0.001, 0.001, 0.001]}>
      ...
      <Billboard follow={true}>
        <mesh 
          ref={cardMeshRef}
          onClick={(e) => { e.stopPropagation(); setActiveProject('skills'); }}
        ...
        <group ref={labelRef} position={[0, 4.2, 0.3]}>
          <Text anchorX="center" anchorY="middle" color={hovered ? "#ff0055" : "#00f0ff"} fontSize={0.4} letterSpacing={0.08}>
            {"[ CLICK TO REGENERATE THE INTEGRATED SKILLS MATRIX ]"}
          </Text>
        </group>
      </Billboard>
    </group>
