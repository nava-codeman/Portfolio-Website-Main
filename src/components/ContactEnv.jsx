...
const ContactEnv = () => {
  ...
  const texture = useTexture('/Spidy%20images/SPIDERMAN12.png');
  ...
  useFrame((state) => {
    ...
    if (cardMeshRef.current) {
      // Animation Profile [Matrix Glow Sweep]
      cardMeshRef.current.material.emissiveIntensity = 1.0 + Math.abs(Math.sin(time * 2)) * 3.0;
    }
  ...
  return (
    <group ref={groupRef} scale={[0.001, 0.001, 0.001]}>
      ...
      <Billboard follow={true}>
        <mesh 
          ref={cardMeshRef}
          onClick={(e) => { e.stopPropagation(); setActiveProject('contact'); }}
        ...
        <group ref={labelRef} position={[0, 4.2, 0.3]}>
          <Text anchorX="center" anchorY="middle" color={hovered ? "#ff0055" : "#00f0ff"} fontSize={0.4} letterSpacing={0.08}>
            {"[ CLICK TO OPEN PORTAL TO CONNECT WITH NAVJYOTI NATH ]"}
          </Text>
        </group>
      </Billboard>
    </group>
