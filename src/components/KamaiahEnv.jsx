...
const KamaiahEnv = () => {
  ...
  const texture = useTexture('/Spidy%20images/SPIDERMAN4.png');
  ...
  useFrame((state) => {
    ...
    if (cardMeshRef.current) {
      // Animation Profile [Diagonal Hover Float]
      cardMeshRef.current.position.x = Math.sin(time) * 0.2;
      cardMeshRef.current.position.y = Math.cos(time) * 0.2;
    }
  ...
  return (
    <group ref={groupRef} scale={[0.001, 0.001, 0.001]}>
      ...
      <Billboard follow={true}>
        <mesh 
          ref={cardMeshRef}
          onClick={(e) => { e.stopPropagation(); setActiveProject('kamaiah'); }}
        ...
        <group ref={labelRef} position={[0, 4.2, 0.3]}>
          <Text anchorX="center" anchorY="middle" color={hovered ? "#ff0055" : "#00f0ff"} fontSize={0.4} letterSpacing={0.08}>
            {"[ CLICK TO DEPLOY KAMAIAH SOFTWARE ENGINEERING LOGS ]"}
          </Text>
        </group>
      </Billboard>
    </group>
